...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        team.save()
        # Update the logic to avoid using exists() and directly handle the queryset
        users_queryset = list(User.objects.all())
        # Ensure the members field is updated only when users_queryset is valid and non-empty
        if users_queryset:
            team.members.add(*users_queryset)
        ...
        # Update the leaderboard creation logic to include unique _id values
        leaderboard_entries = [
        ...
        # Update the workout creation logic to include unique _id values
        workouts = [
